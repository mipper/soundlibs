...
public class VorbisFile {
    ...
    public VorbisFile(final String file)
            throws JOrbisException {
        InputStream is = null;
        try {
            is = new SeekableInputStream(file);
            final int ret = open(is, null, 0);
            if (ret == -1) {
    ...
    public VorbisFile(final InputStream is, final byte[] initial, final int ibytes)
            throws JOrbisException {
        final int ret = open(is, initial, ibytes);
        if (ret == -1) {
        }
    }
    ...
    private int get_data() {
        final int index = oy.buffer(CHUNKSIZE);
        final byte[] buffer = oy._data;
        int bytes = 0;
        try {
    ...
    int fetch_headers(final Info vi, final Comment vc, final int[] serialno, Page og_ptr) {
        ...
        while (i < 3) {
            os.pagein(og_ptr);
            while (i < 3) {
                final int result = os.packetout(op);
                if (result == 0) {
                    break;
                }
                if (result == -1) {
        ...
        while (true) {
            ...
            if (decode_ready) {
                ...
                if (result > 0) {
                    ...
                    if (vb.synthesis(op) == 0) { // lazy check for lazy
                        ...
                        {
                            final int oldsamples = vd.synthesis_pcmout(null, null);
                        ...
                        if (granulepos != -1 && op.e_o_s == 0) {
                            final int link = (seekable ? current_link : 0);
                            final int samples;
            ...
            if (!decode_ready) {
                ...
                if (seekable) {
                    ...
                    final int[] foo = new int[1];
                    final int ret = fetch_headers(vi[0], vc[0], foo, og);
                    current_serialno = foo[0];
                    if (ret != 0) {
                        return ret;
                    }
    ...
    int open_callbacks(final InputStream is, final byte[] initial, final int ibytes//, callbacks callbacks
                      )
            throws JOrbisException {
        final int ret;
        datasource = is;
        ...
        // stream)
        if (initial != null) {
            final int index = oy.buffer(ibytes);
            System.arraycopy(initial, 0, oy._data, index, ibytes);
            oy.wrote(ibytes);
        }
    ...
    public int bitrate_instant() {
        final int _link = (seekable ? current_link : 0);
        if (samptrack == 0) {
            return (-1);
        }
        final int ret = (int) (bittrack / samptrack * vi[_link].rate + .5);
        bittrack = 0.f;
        samptrack = 0.f;
        return (ret);
    }
    ...
    public int pcm_seek(final long pos) {
        ...
        while (pcm_offset < pos) {
            final int target = (int) (pos - pcm_offset);
            final float[][][] _pcm = new float[1][][];
            final int[] _index = new int[getInfo(-1).channels];
    ...
    int time_seek(final float seconds) {
        ...
        {
            final long target = (long) (pcm_total + (seconds - time_total) * vi[link].rate);
    ...
    int read(final byte[] buffer,
             final int length,
             final int bigendianp,
             final int word,
             final int sgned,
             final int[] bitstream) {
        final int host_endian = host_is_big_endian();
        int index = 0;
        ...
        while (true) {
            if (decode_ready) {
                ...
                if (samples != 0) {
                    ...
                    final int channels = getInfo(-1).channels;
    ...
    class SeekableInputStream extends InputStream {
        ...
        SeekableInputStream(final String file)
                throws java.io.IOException {
