...
class DecodeExample {
    ...
    public static void main(final String[] arg) {
        ...
        final SyncState oy = new SyncState(); // sync and verify incoming physical bitstream
        final StreamState os = new StreamState(); // take physical pages, weld into a logical stream of packets
        final Page og = new Page(); // one Ogg bitstream page.  Vorbis packets are inside
        final Packet op = new Packet(); // one raw packet of data for decode

        final Info vi = new Info(); // struct that stores all the static vorbis bitstream settings
        final Comment vc = new Comment(); // struct that stores all the bitstream user comments
        final DspState vd = new DspState(); // central working state for the packet->PCM decoder
