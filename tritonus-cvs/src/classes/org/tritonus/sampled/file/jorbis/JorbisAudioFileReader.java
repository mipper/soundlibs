...
public class JorbisAudioFileReader extends TAudioFileReader {
    ...
    @Override
    protected AudioFileFormat getAudioFileFormat(final InputStream inputStream, final long lFileSizeInBytes)
        ...
        final int index = oggSyncState.buffer(INITAL_READ_LENGTH);
        bytes = inputStream.read(oggSyncState._data, index, INITAL_READ_LENGTH);
        ...
        if (oggPacket.b_o_s == 0) {
            throw new UnsupportedAudioFileException(
                    "not a Vorbis stream: initial packet not marked as beginning of stream");
        }
		...
		*/
        ...
        final int nFrameSize = AudioSystem.NOT_SPECIFIED;
		...
		*/
        ...
        final AudioFormat format = new AudioFormat(new AudioFormat.Encoding("VORBIS"),
                                                   fSampleRate,
