...
public class JorbisFormatConversionProvider extends TEncodingFormatConversionProvider {
    ...
    private static final AudioFormat[] INPUT_FORMATS = {
            ...
            new AudioFormat(VORBIS, -1.0F, -1, 1, -1, -1.0F, false), new AudioFormat(VORBIS,
                                                                                     -1.0F,
                                                                                     -1,
                                                                                     1,
                                                                                     -1,
                                                                                     ...
                                                                                     true),
            // stereo
            new AudioFormat(VORBIS, -1.0F, -1, 2, -1, -1.0F, false), new AudioFormat(VORBIS,
                                                                                     -1.0F,
                                                                                     -1,
                                                                                     2,
                                                                                     -1,
    ...
    private static final AudioFormat[] OUTPUT_FORMATS = {
            // mono, 16 bit signed
            new AudioFormat(PCM_SIGNED, -1.0F, 16, 1, 2, -1.0F, false), new AudioFormat(PCM_SIGNED,
                                                                                        -1.0F,
                                                                                        16,
                                                                                        1,
                                                                                        2,
            ...
            new AudioFormat(PCM_SIGNED, -1.0F, 16, 2, 4, -1.0F, false), new AudioFormat(PCM_SIGNED,
                                                                                        -1.0F,
                                                                                        16,
                                                                                        2,
                                                                                        4,
    ...
    protected AudioFormat getDefaultTargetFormat(final AudioFormat targetFormat, final AudioFormat sourceFormat) {
        ...
        final Iterator iterator = getCollectionTargetFormats().iterator();
        while (iterator.hasNext()) {
            final AudioFormat format = (AudioFormat) iterator.next();
        ...
        if (TDebug.TraceAudioConverter) {
            TDebug.out("JorbisFormatConversionProvider.getDefaultTargetFormat(): new target format: "
                       + newTargetFormat);
        }
        // hacked together...
        ...
        if (TDebug.TraceAudioConverter) {
            TDebug.out("JorbisFormatConversionProvider.getDefaultTargetFormat(): really new target format: "
                       + newTargetFormat);
        }
    ...
    /*private*/public static class DecodedJorbisAudioInputStream extends TAsynchronousFilteredAudioInputStream {
        ...
        */
        ...
        private void processComments() {
            final byte[][] ptr = m_vorbisComment.user_comments;
            String currComment = "";
        ...
        private void decodeDataPacket() {
            ...
            while ((samples = m_vorbisDspState.synthesis_pcmout(_pcmf, _index)) > 0) {
                ...
                for (int nChannel = 0; nChannel < m_vorbisInfo.channels; nChannel++) {
                    ...
                    final int mono = _index[nChannel];
        ...
        private void readOggPacket()
                throws IOException {
            while (true) {
                final int result = m_oggStreamState.packetout(m_oggPacket);
                if (result == 1) {
                    return;
                }
                if (result == -1) {
        ...
        private void readOggPage()
                throws IOException {
            while (true) {
                final int result = m_oggSyncState.pageout(m_oggPage);
                if (result == 1) {
                    return;
                }
                // we need more data from the stream
                final int nIndex = m_oggSyncState.buffer(BUFFER_SIZE);
                // TODO: call stream.read() directly
                final int nBytes = readFromStream(m_oggSyncState._data, nIndex, BUFFER_SIZE);
         ...
         */
        private int readFromStream(final byte[] buffer, final int nStart, final int nLength)
                throws IOException {
